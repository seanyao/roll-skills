...
- This skill requires `workspace_required_read`. Read backlog, features, design, evidence, events, and runtime only through `context.authorities`; any approved metadata mutation must use the same authority paths. Never derive authority from the shell cwd, a repository root, or a nearby `.roll` directory.

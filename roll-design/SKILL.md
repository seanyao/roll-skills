...
- **Evaluation contract (US-SKILL-030)**: every newly split story spec MUST include an `**Evaluation contract:**` block with `expected_evidence` (each item: `kind`, `target`, `proves`) and `scorer_focus`. Genuinely trivial/internal stories may carry a one-item minimal block but must NOT omit the section entirely. This artifact is authored by the planner and consumed by builder and evaluator — see `references/full-contract.md` for the full template and rules.
